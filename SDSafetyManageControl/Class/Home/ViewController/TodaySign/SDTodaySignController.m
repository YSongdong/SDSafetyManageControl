...
@interface SDTodaySignController ()
<
...
@implementation SDTodaySignController
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == self.dataArr.count-1) {
           if (self.dataArr.count == 7) {
                [SVProgressHUD showErrorWithStatus:@"最多上传6张"];
...
#pragma mark -----设置导航栏----
-(void)createNavi{
    [self customNaviItemTitle:@"签到" titleColor:[UIColor colorTextWhiteColor]];
...
-(void)requsetTestCodeImage{
    ...
    param[@"signInUserId"] = [SDUserInfo obtainWithProjectUserID];
    ...
    param[@"type"] = @"addAppProjectSignIn";
    [[KRMainNetTool sharedKRMainNetTool]upLoadData:HTTP_UPDATEWORK_URL params:param.copy andData:self.dataArr waitView:self.view complateHandle:^(id showdata, NSString *error) {
        ...
        if (!error) {
            ...
            SDSignManagerController *signManageVC = [[SDSignManagerController alloc]init];
            [weakSelf.navigationController pushViewController:signManageVC animated:YES];
